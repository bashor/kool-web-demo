...
fun reportException(exception: Throwable, lastAction: String, attach: String ) {
    ...
    if (common.Settings.IS_PRODUCTION) {
        sendExceptionViaITNProxy(bean);
    }
    //todo log
    //        LOG_FOR_EXCEPTIONS.error(ErrorWriter.getExceptionForLog(type, e, description))
}

private fun sendExceptionViaITNProxy(error: ErrorBean) {
    val login = "idea_anonymous"
    val password = "guest"
    try {
        val result = ITNProxy.postNewThread(login, password, error, System.currentTimeMillis().toString(), common.Settings.KOTLIN_COMPILER_VERSION)
